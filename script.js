...
// Fill both currency <select> elements with all available currency codes
function populateCurrencySelects() {
  const from = document.getElementById('fromCurrency');
  const to = document.getElementById('toCurrency');
  if (!from || !to) return;

  // Get all currency codes from exchangeRates and sort alphabetically
  ...
  // Prefer sensible defaults when available
  if (codes.includes('USD')) from.value = 'USD';
  if (codes.includes('EUR')) to.value = 'EUR';
}
...
function init() {
  // Populate currency dropdowns from `exchangeRates` so we get many options
